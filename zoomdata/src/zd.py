...
class Zoomdata(object):
    ...
    def __getVisualization(self, showPickers, time, colors={}):
    ...
    def __render(self, pickers, filters, showPickers, time, colors):
        if(self._source):
            self._pickers = pickers
            self._graphFilters = filters
            iframe = self.__getVisualization(showPickers, time, colors)[0]
            self._renderCount += 1
            return HTML(iframe)
        else:
            print('You need to specify a source: ZD.source = "Source Name"')

    def graph(self, src="", chart= "", pickers=True, conf={}, filters={}, time={}, colors={}):
        ...
        special vars: _variables, _filters, _timeFilter.
            ...
            -colors: Dict(optional). This parameter is used to specify custom colors to the visualization. If the notebook is autogenerated it will be automatically passed.
        """
        if(self.source(src)):
            if(self.__setChart(chart)):
                return self.__render(conf, filters, showPickers=pickers, time=time, colors=colors)
    ...
    def __getWebsocketData(self, source, visual=False, fields=[], exclude=[], rows=10000, config={}, filters=[], time={}):
        ...
        try:
            ...
            if self._source_credentials.get(source, False): # If the source is allready registered
                ...
                check_source_id = self._source_credentials[source][1] # The id of the source
            #Check the source saved data match the one with zoomdata
            if(self._conf['headers']['Authorization']):
